    ...
    <nav role="navigation" aria-label="Pagination Navigation" class="flex gap-2">
         <span>
             ...
             @if ($paginator->onFirstPage())
                 <button type="button" class="rounded-lg border px-3 py-2 bg-white font-semibold text-xs text-gray-700 hover:bg-gray-100 disabled:bg-gray-50 disabled:opacity-75 disabled:cursor-not-allowed disabled:text-gray-500" disabled>
                     Prev
                 </button>
             @else
                 @if(method_exists($paginator,'getCursorName'))
                     <button type="button" dusk="previousPage" wire:key="cursor-{{ $paginator->getCursorName() }}-{{ $paginator->previousCursor()->encode() }}" wire:click="setPage('{{$paginator->previousCursor()->encode()}}','{{ $paginator->getCursorName() }}')" wire:loading.attr="disabled" class="rounded-lg border px-3 py-2 bg-white font-semibold text-xs text-gray-700 hover:bg-gray-100 disabled:bg-gray-50 disabled:opacity-75 disabled:cursor-not-allowed disabled:text-gray-500">
                         Prev
                     </button>
                 @else
